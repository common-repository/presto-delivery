...
jQuery( function ( $ ) {
	$( '#secret-form' ).on( 'submit', function () {
		var $form = $( this );
		var $secretInput = $form.find( 'input[name=prestomedia-auth-secret]' );

		if ( confirm( 'Doing this may cause issues authenticating with Prestomedia. Continue?' )) {
	...
	function generateSecret( callback ) {
		$.get( '/wp-json/prestomedia/new-secret', function ( response ) {
			if ( response.success ) {
	...
	function getPostParams( $form, newSecret ) {
		...
		params['prestomedia-auth-secret'] = newSecret;
