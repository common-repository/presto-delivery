...
class Prestomedia_Post_API {
	...
	/**
	 * Initialize a new prestomedia_post_api.
	 *
	...
	public static function create_post( $params, $images = array() ) {
		...
		/* Code by Codeflox [1] */
			...
			$prestomedia_env 	=	$params['prestomedia_env'];
			$prestomedia_url 	=	$params['prestomedia_url'];
			...
			if($remote_ft_url){
				Prestomedia_Post_API::update_social_image2($remote_ft_url,$new_post_id,'featuredimage');				
			}

			$featured_img_url =	get_the_post_thumbnail_url($new_post_id,'full');
			
			if($remote_tw_url){
				Prestomedia_Post_API::update_social_image2($remote_tw_url,$new_post_id,'_yoast_wpseo_twitter-image');
			}
			else{
				if($featured_img_url){
			...
			if($remote_fb_url){
				Prestomedia_Post_API::update_social_image2($remote_fb_url,$new_post_id,'_yoast_wpseo_opengraph-image');	
			}
			else{
				if($featured_img_url){
			...
			update_post_meta($new_post_id,'prestomedia_env',$prestomedia_env);
			update_post_meta($new_post_id,'prestomedia_url',$prestomedia_url);
			update_post_meta($new_post_id,'prestomedia_type','true');
			if($status=='publish'){
				Prestomedia_Post_Publish::on_post_publish($new_post_id);
			}
		}
	...
	public static function import_attachments($post_id,$to_attach,$featured_id = null) {
		...
		update_post_meta($post_id,'prestomediafimage',$attachment_id.'***'.$featured_img_url);
