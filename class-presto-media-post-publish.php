<?php
class Prestomedia_Post_Publish{
	/**
	 * Base path of this API controller.
	 *
	 * @const string
	 */
	const API_BASE = 'prestomedia';
	/**
	 * The authenticator.
	 *
	 * @var object
	...
	function on_post_publish($post_id){
		...
		$prestomedia_env 		=	get_post_meta($post_id,'prestomedia_env',true);
		$prestomedia_url 		=	get_post_meta($post_id,'prestomedia_url',true);
		$assignmentId 		=	get_post_meta($post_id,'assignment_id',true);
		$secretKey 			=	Prestomedia_Options::get_secret(); //get_post_meta($post_id,'secret_key',true);
		...
		$tags 				=	Prestomedia_Post_Publish::get_tags($post_id);
		...
		$env = $prestomedia_env;
		if ($env == "prod") {			$base_url = "https://my.presto.media";			}
		else if ($env == "staging") {	$base_url = "https://app-staging.my.presto.media";	}
		else {							$base_url = $prestomedia_url;						}
...
add_action('plugins_loaded',array('Prestomedia_Post_Publish','init'));